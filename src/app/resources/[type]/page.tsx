        ...
        <div className="grid gap-4 md:grid-cols-2">
          {docs.map((d) => (
            <a key={d.file} href={d.file} target="_blank" rel="noreferrer" className="rounded-xl border border-slate-200 bg-white p-4">
              <p>{d.name}</p>
              <p className="mt-1 text-sm text-blue-600">Download PDF</p>
            </a>
          ))}
        </div>
        <div className="mt-6"><Link href="/request-quote" className="text-blue-600">Need a custom technical document? Request a quote.</Link></div>
      </section>
    </div>
  );
}
